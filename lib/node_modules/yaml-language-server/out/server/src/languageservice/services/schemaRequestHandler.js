...
exports.schemaRequestHandler = (connection, uri) => {
    ...
    if (scheme === 'file') {
        ...
        return new Promise((c, e) => {
            fs.readFile(fsPath, 'UTF-8', (err, result) => { return err ? e('') : c(result.toString()); });
        });
    }
    ...
    if (scheme === 'http' || scheme === 'https') {
        ...
        return request_light_1.xhr({ url: uri, followRedirects: 5, headers })
            .then(response => { return response.responseText; }, (error) => { return Promise.reject(error.responseText || request_light_1.getErrorStatusDescription(error.status) || error.toString()); });
    }
